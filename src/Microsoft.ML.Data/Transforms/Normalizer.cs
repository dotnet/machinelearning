...
namespace Microsoft.ML.Transforms.Normalizers
{
    ...
    public sealed partial class NormalizingTransformer : OneToOneTransformerBase
    {
        ...
        private sealed class Mapper : OneToOneMapperBase, ISaveAsOnnx, ISaveAsPfa
        {
            ...
            public Mapper(NormalizingTransformer parent, DataSchema schema)
            ...
            {
                var result = new DataSchema.DetachedColumn[_parent.Columns.Length];
                for (int i = 0; i < _parent.Columns.Length; i++)
                    result[i] = new DataSchema.DetachedColumn(_parent.Columns[i].Name, _parent.Columns[i].InputType, MakeMetadata(i));
                return result;
            }

            private DataSchema.Metadata MakeMetadata(int iinfo)
            {
