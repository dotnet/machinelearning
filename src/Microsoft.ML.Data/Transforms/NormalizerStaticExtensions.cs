...
namespace Microsoft.ML.Runtime.Data
{
    ...
    {
        private const long MaxTrain = NormalizerEstimator.Defaults.MaxTrainingExamples;
        private const bool FZ = NormalizerEstimator.Defaults.FixZero;
            ...
            OnFitAffine<ImmutableArray<T>> onFit)
        {
            ...
            return new Impl<T>(input, (src, name) => new NormalizerEstimator.MinMaxColumn(src, name, maxTrainingExamples, fixZero), AffineMapper(onFit));
        }
        ...
        private static NormVector<T> NormalizeByMVCdfCore<T>(Vector<T> input, bool fixZero, bool useLog, bool useCdf, long maxTrainingExamples, Action<IColumnFunction> onFit)
        {
            ...
            {
                if (useLog)
                    return new NormalizerEstimator.LogMeanVarColumn(src, name, maxTrainingExamples, useCdf);
                return new NormalizerEstimator.MeanVarColumn(src, name, maxTrainingExamples, fixZero, useCdf);
            }, onFit);
        }
        ...
        public static NormVector<float> NormalizeByBinning(
            this Vector<float> input, int maxBins = NormalizerEstimator.Defaults.NumBins, bool fixZero = FZ, long maxTrainingExamples = MaxTrain,
        ...
        public static NormVector<double> NormalizeByBinning(
            this Vector<double> input, int maxBins = NormalizerEstimator.Defaults.NumBins, bool fixZero = FZ, long maxTrainingExamples = MaxTrain,
            ...
            OnFitBinned<ImmutableArray<T>> onFit)
        {
            ...
            Contracts.CheckParam(maxTrainingExamples > 1, nameof(maxTrainingExamples), "Must be greater than 1");
            return new Impl<T>(input, (src, name) => new NormalizerEstimator.BinningColumn(src, name, maxTrainingExamples, fixZero, numBins), BinMapper(onFit));
        }
        ...
        private delegate NormalizerEstimator.ColumnBase CreateNormCol(string input, string name);
        ...
        private sealed class Rec : EstimatorReconciler
        {
            ...
            {
                var cols = new NormalizerEstimator.ColumnBase[toOutput.Length];
                ...
                var norm = new NormalizerEstimator(env, cols);
