    ...
    public sealed class FeatureContributionCalculatingTransformer : OneToOneTransformerBase
    {
        ...
        private class Mapper : OneToOneMapperBase
        {
                ...
                : base(parent.Host, parent, schema)
            {
                ...
                if (!schema.TryGetColumnIndex(_parent.ColumnPairs[0].source, out _featureColumnIndex))
                    throw Host.ExceptSchemaMismatch(nameof(schema), "input", _parent.ColumnPairs[0].source);
                ...
                if (_featureColumnType == null || _featureColumnType.ItemType != NumberType.R4)
                    throw Host.ExceptSchemaMismatch(nameof(schema), "feature column", _parent.ColumnPairs[0].source, "Expected type is vector of float.", _featureColumnType.ItemType.ToString());
