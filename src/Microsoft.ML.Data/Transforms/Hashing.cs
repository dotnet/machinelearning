...
namespace Microsoft.ML.Transforms.Conversions
{
    ...
    public sealed class HashingTransformer : OneToOneTransformerBase
    {
        ...
        private ColumnType GetOutputType(Schema inputSchema, ColumnInfo column)
        {
            ...
            inputSchema.TryGetColumnIndex(column.Source, out int srcCol);
            ...
            base(Contracts.CheckRef(env, nameof(env)).Register(RegistrationName), GetColumnPairs(columns))
        {
            ...
            {
                Schema.Column? srcCol = input.Schema.GetColumnOrNull(ColumnPairs[i].source);
                if (srcCol == null)
        ...
        private Delegate GetGetterCore(Row input, int iinfo, out Action disposer)
        {
            ...
            disposer = null;
            input.Schema.TryGetColumnIndex(_columns[iinfo].Source, out int srcCol);
        ...
        private abstract class InvertHashHelper
        {
            ...
            private InvertHashHelper(Row row, ColumnInfo ex)
            {
                ...
                Row = row;
                row.Schema.TryGetColumnIndex(ex.Source, out int srcCol);
                _srcCol = srcCol;
    ...
    public sealed class HashingEstimator : IEstimator<HashingTransformer>
    {
        ...
        /// <paramref name="invertHash"/> specifies the upper bound of the number of distinct input values mapping to a hash that should be retained.
        /// <value>0</value> does not retain any input values. <value>-1</value> retains all input values mapping to each hash.</param>
        public HashingEstimator(IHostEnvironment env, string name, string source = null,
