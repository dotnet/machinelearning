    ...
    public static class SdcaStaticExtensions
    {
            ...
            Action<LinearMulticlassModelParameters> onFit = null)
        {
            ...
            var rec = new TrainerEstimatorReconciler.MulticlassClassificationReconciler<TVal>(
                (env, labelName, featuresName, weightsName) =>
                {
                    var trainer = new SdcaNonCalibratedMulticlassClassificationTrainer(env, labelName, featuresName, weightsName, loss, l2Regularization, l1Threshold, numberOfIterations);
                    if (onFit != null)
                        return trainer.WithOnFitDelegate(trans => onFit(trans.Model));
                    return trainer;
                }, label, features, weights);
            ...
            Action<LinearMulticlassModelParameters> onFit = null)
        {
            ...
            var rec = new TrainerEstimatorReconciler.MulticlassClassificationReconciler<TVal>(
                (env, labelName, featuresName, weightsName) =>
                {
                    ...
                    var trainer = new SdcaNonCalibratedMulticlassClassificationTrainer(env, options);
                    if (onFit != null)
                        return trainer.WithOnFitDelegate(trans => onFit(trans.Model));
                    return trainer;
                }, label, features, weights);
