...
namespace Microsoft.ML.TorchSharp.AutoFormerV2
{
    public class ObjectDetectionTrainer : IEstimator<ObjectDetectionTransformer>
    {
        ...
        internal class Trainer
        {
            ...
            private protected int GetRowCountAndSetLabelCount(IDataView input)
            {
                ...
                foreach (var label in labelCol) // TODO: Fix count here.
                {
                    rowCount++;
            ...
            //    DataViewRowCursor cursor = validationSet.GetRowCursor(validationSet.Schema[Parent.Option.LabelColumnName], validationSet.Schema[Parent.Option.BoundingBoxColumnName], validationSet.Schema[Parent.Option.ImageColumnName], validationSet.Schema[Parent.Option.ConfidenceColumnName]);

            //    var boundingBoxGetter = cursor.GetGetter<VBuffer<float>>(validationSet.Schema[Parent.Option.BoundingBoxColumnName]);
            //    var imageGetter = cursor.GetGetter<MLImage>(validationSet.Schema[Parent.Option.ImageColumnName]);
            //    var labelGetter = cursor.GetGetter<VBuffer<uint>>(validationSet.Schema[Parent.Option.LabelColumnName]);
            ...
            //    while (cursorValid)
            //    {
            //        cursorValid = ValidateStep(cursor, boundingBoxGetter, imageGetter, labelGetter, ref inputTensors, ref numCorrect, ref numRows);
            //    }
            ...
            //        if (cursorValid)
            //        {
            //            (imageTensor, labelTensor) = PrepareData(labelGetter, imageGetter, boundingBoxGetter);
            //            inputTensors.Add();
            ...
            ref List<Tensor> targets)
            {
                ...
                imageTensor = torch.stack(inputTensors);
                targetTensor = torch.stack(targets);
            ...
            private (Tensor image, Tensor Label) PrepareData(ValueGetter<VBuffer<uint>> labelGetter, ValueGetter<MLImage> imageGetter, ValueGetter<VBuffer<float>> boundingBoxGetter)
            {
                using (var _ = torch.NewDisposeScope())
                {
                    ...
                    var midTensor0 = torch.tensor(image.Pixels.ToArray());
                    ...
                    using var transMidTensor = torch.zeros(1, 3, image.Height + padH, image.Width + padW);
                    ...
                    int b = 0;
                    var labelTensor = torch.zeros(1, labels.Length, 5, dtype: ScalarType.Int64);
            ...
            internal static Tensor Normalize(Tensor x)
            {
                using (var _ = torch.NewDisposeScope())
                {
                    var meanTensor = MEAN.ToTensor(new long[4] { 1L, MEAN.Length, 1L, 1L }).to_type(ScalarType.Float32);
                    var stdTensor = STD.ToTensor(new long[4] { 1L, STD.Length, 1L, 1L }).to_type(ScalarType.Float32);
        ...
        private void CheckInputSchema(SchemaShape inputSchema)
        {
            ...
            if (labelCol.ItemType != new VectorDataViewType(new KeyDataViewType(typeof(uint), uint.MaxValue)))
                throw Host.ExceptSchemaMismatch(nameof(inputSchema), "label", Option.LabelColumnName,
            ...
            if (boundingBoxCol.ItemType != new VectorDataViewType(NumberDataViewType.Single))
                throw Host.ExceptSchemaMismatch(nameof(inputSchema), "BoundingBox", Option.BoundingBoxColumnName,
    ...
    public class ObjectDetectionTransformer : RowToRowTransformerBase
    {
        ...
        private protected class ObjDetMapper : MapperBase
        {
            private protected readonly ObjectDetectionTransformer Parent;
                ...
                base(Contracts.CheckRef(parent, nameof(parent)).Host.Register(nameof(ObjDetMapper)), inputSchema, parent)
            {
                ...
                if (inputSchema.TryGetColumnIndex(parent.Options.BoundingBoxColumnName, out var col))
                    InputColIndices.Add(col);

                if (inputSchema.TryGetColumnIndex(parent.Options.ImageColumnName, out col))
                    InputColIndices.Add(col);
            ...
            protected override DataViewSchema.DetachedColumn[] GetOutputColumnsCore()
            {
                ...
                info[0] = new DataViewSchema.DetachedColumn(Parent.Options.PredictedLabelColumnName, new VectorDataViewType(new KeyDataViewType(typeof(uint), Parent.Options.NumberOfClasses)), labelBuilder.ToAnnotations());

                info[1] = new DataViewSchema.DetachedColumn(Parent.Options.ScoreColumnName, new VectorDataViewType(NumberDataViewType.Single), meta.ToAnnotations());

                info[2] = new DataViewSchema.DetachedColumn(Parent.Options.BoundingBoxColumnName, new VectorDataViewType(NumberDataViewType.Single));
                return info;
            ...
            private Delegate MakeScoreGetter(DataViewRow input, IChannel ch, TensorCacher outputCacher)
            {
                ...
                getImage = input.GetGetter<MLImage>(input.Schema[Parent.Options.ImageColumnName]);
            ...
            private Delegate MakeBoundingBoxGetter(DataViewRow input, IChannel ch, TensorCacher outputCacher)
            {
                ...
                {
                    using var disposeScope = torch.NewDisposeScope();
                    var editor = VBufferEditor.Create(ref dst, Parent.Options.NumberOfClasses * 4);
                    UpdateCacheIfNeeded(input.Position, outputCacher, ref image, ref getImage);
            ...
            public override Delegate[] CreateGetters(DataViewRow input, Func<int, bool> activeOutput, out Action disposer)
            {
                ...
                TensorCacher outputCacher = new TensorCacher(Parent.Options.NumberOfClasses);
            ...
            private Tensor PrepInputTensors(ref MLImage image, ValueGetter<MLImage> imageGetter)
            {
                ...
                using (var preprocessScope = torch.NewDisposeScope())
                {
                    ...
                    var padW = 32 - (image.Width % 32);
                    var padH = 32 - (image.Height % 32);
                    var transMidTensor = torch.zeros(1, 3, image.Height + padH, image.Width + padW);
                    transMidTensor[.., .., ..image.Height, ..image.Width] = reMidTensor / 255.0;
            ...
            {
                return col => (activeOutput(0) || activeOutput(1) || activeOutput(2)) && InputColIndices.Any(i => i == col);
            }
        }
    }
}
