...
namespace Microsoft.ML.Transforms
{
    ...
    public static class AlexNetExtension
    {
        ...
        /// </summary>
        public static EstimatorChain<ColumnCopyingTransformer> AlexNet(this DnnImageModelSelector dnnModelContext, IHostEnvironment env, string name, string source)
        {
            return AlexNet(dnnModelContext, env, name, source, Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location), "DnnImageModels"));
        }
        ...
        /// </summary>
        public static EstimatorChain<ColumnCopyingTransformer> AlexNet(this DnnImageModelSelector dnnModelContext, IHostEnvironment env, string name, string source, string modelDir)
        {
            ...
            var endRename = new ColumnCopyingEstimator(env, new[] { (name, "Dropout234_Output_0") });
            ...
            var prepEstimator = new OnnxScoringEstimator(env, Path.Combine(modelDir, "AlexNetPrepOnnx", "AlexNetPreprocess.onnx"), new[] { "PreprocessedInput" }, new[] { "OriginalInput" });
            var mainEstimator = new OnnxScoringEstimator(env, Path.Combine(modelDir, "AlexNetOnnx", "AlexNet.onnx"), new[] { "Dropout234_Output_0" }, new[] { "Input140" });
