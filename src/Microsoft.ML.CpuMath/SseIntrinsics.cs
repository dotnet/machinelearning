...
namespace Microsoft.ML.Runtime.Internal.CpuMath
{
    internal static class SseIntrinsics
    {
        ...
        {
            Contracts.Assert(count <= dst.Length && dst.Length <= src.Length);
            fixed (float* psrc = &MemoryMarshal.GetReference(src))
            fixed (float* pdst = &MemoryMarshal.GetReference(dst))
            {
        ...
        {
            Contracts.Assert(count <= result.Length && result.Length <= src.Length && result.Length <= dst.Length);
            fixed (float* psrc = &MemoryMarshal.GetReference(src))
            fixed (float* pdst = &MemoryMarshal.GetReference(dst))
        ...
        {
            Contracts.Assert(count <= idx.Length && idx.Length <= src.Length && idx.Length <= dst.Length);
            fixed (float* psrc = &MemoryMarshal.GetReference(src))
        ...
        {
            Contracts.Assert(count <= dst.Length && dst.Length <= src1.Length && dst.Length <= src2.Length);
