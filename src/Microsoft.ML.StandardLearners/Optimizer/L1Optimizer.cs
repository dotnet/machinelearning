    ...
    public sealed class L1Optimizer : Optimizer
    {
        ...
        internal override OptimizerState MakeState(IChannel ch, IProgressChannelProvider progress, DifferentiableFunction function, ref VBuffer<Float> initial)
        {
            ...
            if (_l1weight > 0 && _biasCount < initial.Length)
                return new L1OptimizerState(ch, progress, function, initial, M, TotalMemoryLimit, _biasCount, _l1weight, KeepDense, EnforceNonNegativity);
            return new FunctionOptimizerState(ch, progress, function, initial, M, TotalMemoryLimit, KeepDense, EnforceNonNegativity);
        }
        ...
        public sealed class L1OptimizerState : OptimizerState
        {
            ...
            internal L1OptimizerState(IChannel ch, IProgressChannelProvider progress, DifferentiableFunction function, in ReadOnlyVBuffer<Float> initial, int m, long totalMemLimit,
            ...
            private void MakeSteepestDescDir()
            {
                if (!EnforceNonNegativity)
                {
                    VBufferUtils.ApplyInto(_x, _grad, ref _steepestDescDir,
                        (int ind, float xVal, float gradVal) =>
                        {
                            if (ind < _biasCount)
                                return -gradVal;
                            if (xVal < 0)
            ...
            {
                VectorUtils.AddMultInto(_x, alpha, _dir, ref _newX);

                if (!EnforceNonNegativity)
                {
                    VBufferUtils.ApplyWith(_x, ref _newX,
            ...
            {
                Float dirDeriv = -VectorUtils.DotProduct(_dir, _steepestDescDir);
                ...
                GetNextPoint(alpha);
                Float unnormCos = VectorUtils.DotProduct(_steepestDescDir, _newX) - VectorUtils.DotProduct(_steepestDescDir, _x);
                if (unnormCos < 0)
                {
                    VBufferUtils.ApplyWith(_steepestDescDir, ref _dir,
                    ...
                    GetNextPoint(alpha);
                    unnormCos = VectorUtils.DotProduct(_steepestDescDir, _newX) - VectorUtils.DotProduct(_steepestDescDir, _x);
                }
