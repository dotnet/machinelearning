...
namespace Microsoft.ML.Data
{
    ...
    internal sealed class RoleMappedSchema
    {
        ...
        private RoleMappedSchema(DataSchema schema, Dictionary<string, List<DataSchema.Column>> map)
            : this(schema, Copy(map))
        {
        }

        private static void Add(Dictionary<string, List<DataSchema.Column>> map, ColumnRole role, DataSchema.Column column)
        {
            Contracts.AssertValue(map);
            ...
            {
                list = new List<DataSchema.Column>();
                map.Add(role.Value, list);
            }
            list.Add(column);
        }

        private static Dictionary<string, List<DataSchema.Column>> MapFromNames(DataSchema schema, IEnumerable<KeyValuePair<ColumnRole, string>> roles, bool opt = false)
        {
            Contracts.AssertValue(schema);
            Contracts.AssertValue(roles);

            var map = new Dictionary<string, List<DataSchema.Column>>();
        ...
        /// it returns null.
        /// </summary>
        public IReadOnlyList<DataSchema.Column> GetColumns(ColumnRole role)
            => _map.TryGetValue(role.Value, out var list) ? list : null;

        /// <summary>
        /// An enumerable over all role-column associations within this object.
        /// </summary>
        public IEnumerable<KeyValuePair<ColumnRole, DataSchema.Column>> GetColumnRoles()
        {
            foreach (var roleAndList in _map)
            {
                foreach (var info in roleAndList.Value)
                    yield return new KeyValuePair<ColumnRole, DataSchema.Column>(roleAndList.Key, info);
            }
        }
        ...
        /// <summary>
        /// Returns the <see cref="DataSchema.Column"/> corresponding to <paramref name="role"/> if there is
        ...
        public DataSchema.Column GetUniqueColumn(ColumnRole role)
        {
        ...
        private static Dictionary<string, IReadOnlyList<DataSchema.Column>> Copy(Dictionary<string, List<DataSchema.Column>> map)
        {
            var copy = new Dictionary<string, IReadOnlyList<DataSchema.Column>>(map.Count);
        ...
        public RoleMappedSchema(DataSchema schema, bool opt = false, params KeyValuePair<ColumnRole, string>[] roles)
        ...
        /// values for the column names that does not appear in <paramref name="schema"/> will result in an exception being thrown,
        /// but if <c>true</c> such values will be ignored</param>
        public RoleMappedSchema(DataSchema schema, IEnumerable<KeyValuePair<ColumnRole, string>> roles, bool opt = false)
        ...
        /// values for the column names that does not appear in <paramref name="schema"/> will result in an exception being thrown,
        /// but if <c>true</c> such values will be ignored</param>
        public RoleMappedSchema(DataSchema schema, string label, string feature,
