...
namespace Microsoft.ML.Transforms.FeatureSelection
{
    ...
    public sealed class MutualInformationFeatureSelectingEstimator : IEstimator<ITransformer>
    {
        ...
        /// </format>
        /// </example>
        public MutualInformationFeatureSelectingEstimator(IHostEnvironment env, string name, string source = null,
            string labelColumn = Defaults.LabelColumn, int slotsInOutput = Defaults.SlotsInOutput, int numBins = Defaults.NumBins)
            : this(env, labelColumn, slotsInOutput, numBins, (name, source ?? name))
        {
        }
        ...
        public ITransformer Fit(IDataView input)
        {
            ...
            using (var ch = _host.Start("Selecting Slots"))
            {
                ...
                (string name, string source)[] copyColumnPairs;
                CreateDropAndCopyColumns(colArr.Length, scores, threshold, tiedScoresToKeep, _columns.Where(col => colSet.Contains(col.source)).ToArray(), out int[] selectedCount, out dropSlotsColumns, out copyColumnPairs);
            ...
            foreach (var colPair in _columns)
            {
                ...
                if (!MutualInformationFeatureSelectionUtils.IsValidColumnType(col.ItemType))
                    throw _host.ExceptUserArg(nameof(inputSchema),
                        "Column '{0}' does not have compatible type. Expected types are float, double, int, bool and key.", colPair.source);
        ...
        internal static IDataTransform Create(IHostEnvironment env, Arguments args, IDataView input)
        {
            ...
            (string name, string source)[] cols = args.Column.Select(col => (col, col)).ToArray();
        ...
        private static void CreateDropAndCopyColumns(int size, float[][] scores, float threshold, int tiedScoresToKeep, (string name, string source)[] cols,
            out int[] selectedCount, out SlotsDroppingTransformer.ColumnInfo[] dropSlotsColumns, out (string name, string source)[] copyColumnsPairs)
        {
            ...
            for (int i = 0; i < size; i++)
            {
                ...
                if (slots.Count <= 0)
                    ...
                    dropCols.Add(new SlotsDroppingTransformer.ColumnInfo(cols[i].name, cols[i].source, slots.ToArray()));
            }
