...
namespace Microsoft.ML.Transforms.FeatureSelection
{
    ...
    public sealed class MutualInformationFeatureSelectingEstimator : IEstimator<ITransformer>
    {
        ...
        /// </format>
        /// </example>
        public MutualInformationFeatureSelectingEstimator(IHostEnvironment env, string outputColumnName, string sourceColumnName = null,
            string labelColumn = Defaults.LabelColumn, int slotsInOutput = Defaults.SlotsInOutput, int numBins = Defaults.NumBins)
            : this(env, labelColumn, slotsInOutput, numBins, (outputColumnName, sourceColumnName ?? outputColumnName))
        {
        }
        ...
        public ITransformer Fit(IDataView input)
        {
            ...
            using (var ch = _host.Start("Selecting Slots"))
            {
                ...
                {
                    if (!colSet.Add(col.sourceColumnName))
                ...
                (string outputColumnName, string sourceColumnName)[] copyColumnPairs;
                CreateDropAndCopyColumns(colArr.Length, scores, threshold, tiedScoresToKeep, _columns.Where(col => colSet.Contains(col.sourceColumnName)).ToArray(), out int[] selectedCount, out dropSlotsColumns, out copyColumnPairs);
            ...
            foreach (var colPair in _columns)
            {
                ...
                if (!MutualInformationFeatureSelectionUtils.IsValidColumnType(col.ItemType))
                    throw _host.ExceptUserArg(nameof(inputSchema),
                        "Column '{0}' does not have compatible type. Expected types are float, double, int, bool and key.", colPair.sourceColumnName);
        ...
        internal static IDataTransform Create(IHostEnvironment env, Arguments args, IDataView input)
        {
            ...
            (string outputColumnName, string sourceColumnName)[] cols = args.Column.Select(col => (col, col)).ToArray();
        ...
        private static void CreateDropAndCopyColumns(int size, float[][] scores, float threshold, int tiedScoresToKeep, (string outputColumnName, string sourceColumnName)[] cols,
            out int[] selectedCount, out SlotsDroppingTransformer.ColumnInfo[] dropSlotsColumns, out (string outputColumnName, string sourceColumnName)[] copyColumnsPairs)
        {
            ...
            for (int i = 0; i < size; i++)
            {
                ...
                if (slots.Count <= 0)
                    ...
                    dropCols.Add(new SlotsDroppingTransformer.ColumnInfo(cols[i].outputColumnName, cols[i].sourceColumnName, slots.ToArray()));
            }
