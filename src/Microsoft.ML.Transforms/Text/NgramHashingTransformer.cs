...
namespace Microsoft.ML.Transforms.Text
{
    ...
    public sealed class NgramHashingTransformer : RowToRowTransformerBase
    {
        ...
        private sealed class Mapper : MapperBase
        {
            ...
            public Mapper(NgramHashingTransformer parent, DataSchema inputSchema, FinderDecorator decorator = null) :
            ...
            {
                var result = new DataSchema.DetachedColumn[_parent._columns.Length];
                for (int i = 0; i < _parent._columns.Length; i++)
                {
                    var builder = new MetadataBuilder();
                    AddMetadata(i, builder);
                    result[i] = new DataSchema.DetachedColumn(_parent._columns[i].Name, _types[i], builder.GetMetadata());
                }
                return result;
            }
        ...
        private sealed class InvertHashHelper
        {
            ...
            public InvertHashHelper(NgramHashingTransformer parent, DataSchema inputSchema, string[][] friendlyNames, IEnumerable<DataSchema.Column> columnsNeeded, int[] invertHashMaxCounts)
            {
