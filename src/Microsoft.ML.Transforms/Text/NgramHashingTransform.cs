...
[assembly: LoadableClass(typeof(NgramHashingTransform), typeof(NgramHashingTransformer.Arguments), typeof(SignatureDataTransform),
    "Ngram Hash Transform", "NgramHashTransform", "NgramHash")]

[assembly: LoadableClass(typeof(NgramHashingTransform), null, typeof(SignatureLoadDataTransform),
    "Ngram Hash Transform", NgramHashingTransform.LoaderSignature)]

[assembly: LoadableClass(NgramHashingTransform.Summary, typeof(NgramHashingTransform), null, typeof(SignatureLoadModel),
    "Ngram Hash Transform", NgramExtractingTransformer.LoaderSignature)]

[assembly: LoadableClass(typeof(IRowMapper), typeof(NgramHashingTransform), null, typeof(SignatureLoadRowMapper),
    "Ngram Hash Transform", NgramHashingTransform.LoaderSignature)]
namespace Microsoft.ML.Transforms.Text
{
    public sealed class NgramHashingTransform : RowToRowTransformerBase
    {
        ...
        public class ColumnInfo
        {
            ...
            internal readonly string[] FriendlyNames;
        ...
        //IVAN: public constructor for case without invertHashing.

        internal NgramHashingTransform(IHostEnvironment env, IDataView input, params ColumnInfo[] columns) :
            base(Contracts.CheckRef(env, nameof(env)).Register(nameof(NgramHashingTransform)))
        {
            ...
            {
                Func<int, bool> inputPred = (int x) => true;
                var active = new bool[1];
                string[][] friendlyNames = _columns.Select(c => c.FriendlyNames).ToArray();
                var helper = new InvertHashHelper(this, input.Schema, friendlyNames, inputPred, invertHashMaxCounts.ToArray());
        ...
        private NgramHashingTransform(IHostEnvironment env, ModelLoadContext ctx) :
            base(Contracts.CheckRef(env, nameof(env)).Register(nameof(NgramHashingTransform)))
        {
            var columnsLenght = ctx.Reader.Read();
        ...
        internal static IDataTransform Create(IHostEnvironment env, Arguments args, IDataView input)
        {
            ...
            return new NgramHashingTransform(env, input, cols).MakeDataTransform(input);
        }

        // Factory method for SignatureLoadModel.
        private static NgramHashingTransform Create(IHostEnvironment env, ModelLoadContext ctx)
        {
            Contracts.CheckValue(env, nameof(env));
            var host = env.Register(nameof(NgramHashingTransform));

            host.CheckValue(ctx, nameof(ctx));
            ctx.CheckAtModel(GetVersionInfo());
            return new NgramHashingTransform(host, ctx);
        }
        ...
        {
            private readonly NgramHashingTransform _parent;
            private readonly ColumnType[] _types;
            private readonly int[][] _srcIndices;
            private readonly ColumnType[][] _srcTypes;

            public Mapper(NgramHashingTransform parent, Schema inputSchema) :
            ...
            protected override Delegate MakeGetter(IRow input, int iinfo, Func<int, bool> activeOutput, out Action disposer)
            {
                ...
                disposer = null;
                int srcCount = _srcIndices.Length;
        ...
        private sealed class InvertHashHelper
        {
            ...
            public InvertHashHelper(NgramHashingTransform parent, Schema inputSchema, string[][] friendlyNames, Func<int, bool> inputPred, int[] invertHashMaxCounts)
            {
    ...
    /// </summary>
    public sealed class NgramHashingEstimator : IEstimator<NgramHashingTransform>
    {
        ...
        private readonly NgramHashingTransform.ColumnInfo[] _columns;
        ...
        ///
        /// <see cref="NgramHashingEstimator"/> is different from <see cref="WordHashBagEstimator"/> in a way that <see cref="NgramHashEstimator"/>
        /// takes tokenized text as input while <see cref="WordHashBagEstimator"/> tokenizes text internally.
        /// </summary>
        ...
        public NgramHashingEstimator(IHostEnvironment env,
            ...
            bool ordered = true,
            int invertHash = 0)
             : this(env, columns.Select(x => new NgramHashingTransform.ColumnInfo(x.inputs, x.output, ngramLength, skipLength, allLengths, hashBits, seed, ordered, invertHash)).ToArray())
        {
        ...
        public NgramHashingEstimator(IHostEnvironment env, params NgramHashingTransform.ColumnInfo[] columns)
        {
            Contracts.CheckValue(env, nameof(env));
        ...
        public NgramHashingTransform Fit(IDataView input) => new NgramHashingTransform(_host, input, _columns);
    }
