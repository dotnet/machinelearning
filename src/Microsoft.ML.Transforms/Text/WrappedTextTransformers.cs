...
namespace Microsoft.ML.Transforms.Text
{
    ...
    public sealed class WordBagEstimator : TrainedWrapperEstimatorBase
    {
        ...
        public WordBagEstimator(IHostEnvironment env,
            ...
            : this(env, name, new[] { source ?? name }, ngramLength, skipLength, allLengths, maxNumTerms, weighting)
        {
        }
        ...
        public WordBagEstimator(IHostEnvironment env,
            ...
            int maxNumTerms = 10000000,
            NgramExtractingEstimator.WeightingCriteria weighting = NgramExtractingEstimator.WeightingCriteria.Tf)
            : this(env, new[] { (name, sources) }, ngramLength, skipLength, allLengths, maxNumTerms, weighting)
        {
        }
        ...
        {
            foreach (var (name, source) in columns)
            {
                Host.CheckUserArg(Utils.Size(name) > 0, nameof(columns));
                Host.CheckValue(source, nameof(columns));
            }
        ...
        public override TransformWrapper Fit(IDataView input)
        {
            ...
            {
                Column = _columns.Select(x => new WordBagBuildingTransformer.Column { Name = x.name, Source = x.sources }).ToArray(),
    ...
    public sealed class WordHashBagEstimator : TrainedWrapperEstimatorBase
    {
        ...
        /// </summary>
        /// <param name="env">The environment.</param>
        /// <param name="name">The column containing bag of word vector. Null means <paramref name="source"/> is replaced.</param>
        /// <param name="source">The column containing text to compute bag of word vector.</param>
