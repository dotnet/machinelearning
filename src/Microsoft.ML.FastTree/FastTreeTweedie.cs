...
namespace Microsoft.ML.Trainers.FastTree
{
         ...
         : BoostingFastTreeTrainerBase<FastTreeTweedieTrainer.Options, RegressionPredictionTransformer<FastTreeTweedieModelParameters>, FastTreeTweedieModelParameters>
    {
        ...
        protected override void CheckArgs(IChannel ch)
        {
            ...
            if (OptionsBase.EarlyStoppingMetrics > 0)
                ch.Warning("For Tweedie regression, early stopping does not yet use the Tweedie distribution.");

            ch.CheckUserArg((OptionsBase.EarlyStoppingRule == null && !OptionsBase.EnablePruning) || (OptionsBase.EarlyStoppingMetrics >= 1 && OptionsBase.EarlyStoppingMetrics <= 2), nameof(OptionsBase.EarlyStoppingMetrics),
                    "earlyStoppingMetrics should be 1 or 2. (1: L1, 2: L2)");
        }

        protected override ObjectiveFunctionBase ConstructObjFunc(IChannel ch)
        {
            return new ObjectiveImpl(TrainSet, OptionsBase);
        }
        ...
        private protected override OptimizationAlgorithm ConstructOptimizationAlgorithm(IChannel ch)
        {
            ...
            if (OptionsBase.UseLineSearch)
            {
                ...
                optimizationAlgorithm.AdjustTreeOutputsOverride = new LineSearch(lossCalculator, 1 /*L2 error*/, OptionsBase.NumPostBracketSteps, OptionsBase.MinStepSize);
            }
        ...
        {
            Host.CheckUserArg(1 <= OptionsBase.Index && OptionsBase.Index <= 2, nameof(OptionsBase.Index), "Must be in the range [1, 2]");
        ...
        protected override void InitializeTests()
        {
            ...
            if (OptionsBase.PrintTrainValidGraph && _trainRegressionTest == null)
            {
                Test trainRegressionTest = new RegressionTest(ConstructScoreTracker(TrainSet));
                _trainRegressionTest = trainRegressionTest;
            }

            if (OptionsBase.PrintTrainValidGraph && _testRegressionTest == null && TestSets != null && TestSets.Length > 0)
                _testRegressionTest = new RegressionTest(ConstructScoreTracker(TestSets[0]));

            // Add early stopping if appropriate.
            TrainTest = new RegressionTest(ConstructScoreTracker(TrainSet), OptionsBase.EarlyStoppingMetrics);
            if (ValidSet != null)
                ValidTest = new RegressionTest(ConstructScoreTracker(ValidSet), OptionsBase.EarlyStoppingMetrics);

            if (OptionsBase.EnablePruning && ValidTest != null)
            {
                if (OptionsBase.UseTolerantPruning) // Use simple early stopping condition.
        ...
        protected override RegressionPredictionTransformer<FastTreeTweedieModelParameters> MakeTransformer(FastTreeTweedieModelParameters model, DataViewSchema trainSchema)
