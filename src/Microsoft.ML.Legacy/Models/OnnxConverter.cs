...
namespace Microsoft.ML.Legacy.Models
{
    public sealed partial class OnnxConverter
    {
        ...
        /// <para><c><see cref="Onnx"/></c> indicates the file to write the ONNX protocol buffer file to. This is required.</para>
        /// <para><c><see cref="Json"/></c> indicates the file to write the JSON representation of the ONNX model. This is optional.</para>
        /// <para><c><see cref="Name"/></c> indicates the name property in the ONNX model. If left unspecified, it will
        /// be the extension-less name of the file specified in the onnx indicates the protocol buffer file
        /// to write the ONNX representation to.</para>
        /// <para><c><see cref="Domain"/></c> indicates the domain name of the model. ONNX uses reverse domain name space indicators.
        /// For example com.microsoft.cognitiveservices. This is a required field.</para>
        /// <para><c><see cref="InputsToDrop"/></c> is a string array of input column names to omit from the input mapping.
        /// A common scenario might be to drop the label column, for instance, since it may not be practically
        /// useful for the pipeline. Note that any columns depending on these naturally cannot be saved.</para>
        /// <para><c><see cref="OutputsToDrop"/></c> is similar, except for the output schema. Note that the pipeline handler
        /// is currently not intelligent enough to drop intermediate calculations that produce this value: this will
        /// merely omit that value from the actual output.</para>
        ///
