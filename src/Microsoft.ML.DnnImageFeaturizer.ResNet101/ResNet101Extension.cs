...
namespace Microsoft.ML.Transforms
{
    ...
    public static class ResNet101Extension
    {
        ...
        /// </summary>
        public static EstimatorChain<ColumnCopyingTransformer> ResNet101(this DnnImageModelSelector dnnModelContext, IHostEnvironment env, string name, string source)
        {
            return ResNet101(dnnModelContext, env, name, source, Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location), "DnnImageModels"));
        }
        ...
        /// </summary>
        public static EstimatorChain<ColumnCopyingTransformer> ResNet101(this DnnImageModelSelector dnnModelContext, IHostEnvironment env, string name, string source, string modelDir)
        {
            ...
            var endRename = new ColumnCopyingEstimator(env, new[] { (name, "Pooling2286_Output_0") });
            ...
            var mainEstimator = new OnnxScoringEstimator(env, Path.Combine(modelDir, "ResNet101Onnx", "ResNet101.onnx"), new[] { "Pooling2286_Output_0" }, new[] { "Input1600" });
