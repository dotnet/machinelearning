    ...
    public sealed class NormalizerTests : TestDataPipeBase
    {
        ...
        public void NormalizerWorkout()
        {
            ...
            using (var ch = Env.Start("save"))
            {
                ...
                {
                    var dataView = SelectColumnsTransformer.CreateDrop(Env, est.Fit(data).Transform(data), "float0");
        ...
        public void LpGcNormAndWhiteningWorkout()
        {
            ...
            using (var ch = Env.Start("save"))
            {
                ...
                savedData = SelectColumnsTransformer.CreateKeep(Env, savedData, new[] { "lpnorm", "gcnorm", "whitened" });
        ...
        public void WhiteningWorkout()
        {
            ...
            using (var ch = Env.Start("save"))
            {
                var saver = new TextSaver(Env, new TextSaver.Arguments { Silent = true, OutputHeader = false });
                IDataView savedData = TakeFilter.Create(Env, est.Fit(data.AsDynamic).Transform(data.AsDynamic), 4);
                savedData = SelectColumnsTransformer.CreateKeep(Env, savedData, new[] { "whitened1", "whitened2" });
