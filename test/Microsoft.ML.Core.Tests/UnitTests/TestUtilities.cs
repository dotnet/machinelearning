            ...
            try
            {
                ...
                using (var errWriter = new StringWriter(sbErr))
                {
                    ...
                    using (var ch = env.Start("Downloading"))
                    {
                        ...
                        if (!sbErr.ToString().Contains("Could not download"))
                            Fail($"Expected error message to contain the string \"Could not download\". Instead: {sbErr.ToString()}");
                ...
                if (!Uri.TryCreate(@"\\ct01\public\Samples", UriKind.Absolute, out var goodUri))
                    Fail("Uri could not be created");
                ...
                // Bad url.
                if (!Uri.TryCreate("https://ejfiakaa.mswoekg/tlc-resources/", UriKind.Absolute, out badUri))
                    Fail("Uri could not be created");
                ...
                using (var errWriter = new StringWriter(sbErr))
                {
                    ...
                    using (var ch = env.Start("Downloading"))
                    {
                        ...
                        if (!sbErr.ToString().Contains("Could not download"))
                            Fail("Error message should contain 'Could not download'");
#if !CORECLR
