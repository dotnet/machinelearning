...
namespace Microsoft.ML.RunTests
{
    public partial class TestEntryPoints : CoreBaseTestClass
    {
        ...
        // This test mirrors EntryPointPipelineEnsemble, but loads an older model with BinaryLoader.GetVersionInfo().verWrittenCur = 0x00010003 
        [Fact]
        public void EntryPointBackwardCompatibility()
        {
            var dataView = GetBreastCancerDataView();
            const int nModels = 5;
            var splitOutput = CVSplit.Split(Env, new CVSplit.Input { Data = dataView, NumFolds = nModels + 1 });
            ...
            var olderModelPath = GetDataPath("backcompat/PipelineEnsembleModel-BinaryLoaderVersion-0x00010003.zip");

            PredictorModel loadedOlderModel;
            using (var file = Env.OpenInputFile(olderModelPath))
            using (var strm = file.OpenReadStream())
                loadedOlderModel = new PredictorModelImpl(Env, strm);

            var scoredFromOlderModel = ScoreModel.Score(Env,
                new ScoreModel.Input()
                {
                    Data = splitOutput.TestData[nModels],
                    PredictorModel = loadedOlderModel
            ...
            using (var cursOlderModel = scoredFromOlderModel.GetRowCursorForAllColumns())
            {
                ...
                scoreColumn = cursOlderModel.Schema.GetColumnOrNull(AnnotationUtils.Const.ScoreValueKind.Score);
                Assert.True(scoreColumn.HasValue);
                var getterSaved = cursOlderModel.GetGetter<Single>(scoreColumn.Value);
                ...
                while (cursReg.MoveNext())
                {
                    ...
                    Assert.True(cursOlderModel.MoveNext());
                ...
                Assert.False(cursOlderModel.MoveNext());
