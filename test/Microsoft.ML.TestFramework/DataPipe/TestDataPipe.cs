...
namespace Microsoft.ML.RunTests
{
    public sealed partial class TestDataPipe : TestDataPipeBase
    {
        ...
        private void TestHashTransformHelper<T>(T[] data, uint[] results, NumberDataViewType type)
        {
            ...
            var hashTransform = new HashingTransformer(Env, new HashingEstimator.Options("F1", "F1", 5, 42)).Transform(srcView);
        ...
        {
            var srcView = builder.GetDataView();
            var hashTransform = new HashingTransformer(Env, new HashingEstimator.Options("F1V", "F1V", 5, 42)).Transform(srcView);
            using (var cursor = hashTransform.GetRowCursorForAllColumns())
            {
