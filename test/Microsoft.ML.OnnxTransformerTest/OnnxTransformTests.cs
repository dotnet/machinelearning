...
namespace Microsoft.ML.Tests
{
    public class OnnxTransformTests : TestDataPipeBase
    {
        ...
        public void TestOnnxTransformWithCustomShapes()
        {
            ...
            var dataView = ML.Data.LoadFromEnumerable(dataPoints);
            var transformedDataView = ML.Transforms.ApplyOnnxModel(
                new[] { nameof(PredictionWithCustomShape.argmax) }, new[] { nameof(InputWithCustomShape.input) },
                modelFile, shapeDictionary: shapeDictionary).Fit(dataView).Transform(dataView);
        ...
        private void TryModwlWithCustomShapesHelper(IDictionary<string, int[]> shapeDictionary)
        {
            ...
            var pipeline = ML.Transforms.ApplyOnnxModel(new[] { "outa", "outb" }, new[] { "ina", "inb" },
                modelFile, shapeDictionary: shapeDictionary);
        }
        ...
        public void SpecifyOnnxShapes()
        {
            ...
            try
            {
                TryModwlWithCustomShapesHelper(shapeDictionary);
            }
            catch
            {
                somethingWrong = true;
