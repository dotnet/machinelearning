...
namespace Microsoft.ML.CodeAnalyzer.Tests.Helpers
{
        ...
        where TDiag : DiagnosticAnalyzer, new()
    {
        ...
        /// </summary>
        protected virtual TDiag GetCSharpDiagnosticAnalyzer()
            => new TDiag();
        ...
        /// <summary>
        /// Called to test a C# DiagnosticAnalyzer when applied on the single input string as a source
        ...
        /// <summary>
        /// Called to test a C# DiagnosticAnalyzer when applied on the input strings as a source
        /// Note: input a DiagnosticResult for each Diagnostic expected
        /// </summary>
        ...
        private static readonly MetadataReference CorlibReference = RefFromType<object>();
        // I'd hope for a better way for these two.
        private static readonly MetadataReference StandardReference = MetadataReference.CreateFromFile(Assembly.Load("netstandard, Version=2.0.0.0").Location);
        private static readonly MetadataReference RuntimeReference = MetadataReference.CreateFromFile(Assembly.Load("System.Runtime, Version=0.0.0.0").Location);
        ...
        internal static Project CreateProject(string projectName, ref Solution solution, params string[] sources)
        {
            ...
            {
                string sourceTrimmed = source.TrimStart('\r', '\n');
                string newFileName = fileNamePrefix + count + "." + CSharpDefaultFileExt;
                DocumentId documentId = DocumentId.CreateNewId(projectId, debugName: newFileName);
                solution = solution.AddDocument(documentId, newFileName, SourceText.From(sourceTrimmed));
                count++;
            }
            var proj = solution.GetProject(projectId);
            proj = proj.WithCompilationOptions(proj.CompilationOptions.WithOutputKind(OutputKind.DynamicallyLinkedLibrary));
            return proj;
        }
        #endregion
    }
