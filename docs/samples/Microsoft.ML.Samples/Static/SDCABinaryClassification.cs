...
namespace Samples.Static
{
    public class SdcaBinaryClassificationExample
    {
        public static void Example()
        {
            ...
            var metrics = mlContext.BinaryClassification.EvaluateWithPRCurve(
                dataWithPredictions, row => row.Label, row => row.Score,
            ...
            Console.WriteLine(
                $"Negative Precision: {metrics.NegativePrecision}"); // 0.87
            Console.WriteLine($"Negative Recall: {metrics.NegativeRecall}"); // 0.91
            Console.WriteLine(
                $"Positive Precision: {metrics.PositivePrecision}"); // 0.65
