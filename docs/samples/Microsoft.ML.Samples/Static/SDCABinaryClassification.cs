...
namespace Microsoft.ML.Samples.Static
{
    public class SdcaBinaryClassificationExample
    {
        public static void SdcaBinaryClassification()
        {
            ...
            var metrics = mlContext.BinaryClassification.EvaluateWithPRCurve(dataWithPredictions, row => row.Label, row => row.Score, out IEnumerable<PrecisionRecallMetrics> prCurve);
            ...
            while (prData.MoveNext())
            {
                Console.Write($"Threshold: {prData.Current.Threshold} ");
                Console.Write($"Precision: {prData.Current.Precision} ");
                Console.Write($"Recall: {prData.Current.Recall} ");
                Console.WriteLine($"FPR: {prData.Current.FalsePositiveRate}");
            }
        }
    }
}