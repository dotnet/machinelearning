    ...
    public static class PermutationFeatureImportance
    {
        public static void Example()
        {
            ...
            var featureColumns = new string[]
                {nameof(Data.Feature1), nameof(Data.Feature2)};
            ...
            var sortedIndices = permutationMetrics.Select((metrics, index) =>
                    new {index, metrics.MicroAccuracy})
            ...
            var microAccuracy =
                permutationMetrics.Select(x => x.MicroAccuracy).ToArray();
            ...
            int seed = 1)
        {
            ...
            for (int i = 0; i < nExamples; i++)
            {
                ...
                var value = (float) (bias + weight1 * data.Feature1 +
                                     weight2 * data.Feature2 + rng.NextDouble() -
                                     0.5);
                if (value < max / 3)
                    data.Label = 0;
