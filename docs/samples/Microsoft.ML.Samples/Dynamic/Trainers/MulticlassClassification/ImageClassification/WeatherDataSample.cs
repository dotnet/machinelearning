...
namespace Samples.Dynamic
{
    public class WeatherDataSample
    {
        public static void Example()
        {
            ...
            string finalImagesFolderNameTrain = "WeatherData\\train";
            string fullImagesetFolderPathTrain = Path.Combine(
                imagesDownloadFolderPath, finalImagesFolderNameTrain);

            string finalImagesFolderNameTest = "WeatherData\\test";
            string fullImagesetFolderPathTest = Path.Combine(
                imagesDownloadFolderPath, finalImagesFolderNameTest);

            string finalImagesFolderNameValid = "WeatherData\\validation";
            string fullImagesetFolderPathValid = Path.Combine(
                imagesDownloadFolderPath, finalImagesFolderNameValid);
            ...
            try
            {
                ...
                IEnumerable<ImageData> valid_images = LoadImagesFromDirectory(
                    folder: fullImagesetFolderPathValid, useFolderNameAsLabel: true);
                IDataView validDataset = mlContext.Data.LoadFromEnumerable(valid_images);
                validDataset = mlContext.Transforms.Conversion
                        .MapValueToKey("Label", keyOrdinality: ValueToKeyMappingEstimator.KeyOrdinality.ByValue)
                    .Append(mlContext.Transforms.LoadRawImageBytes("Image",
                                fullImagesetFolderPathValid, "ImagePath"))
                    .Fit(validDataset)
                    .Transform(validDataset);
