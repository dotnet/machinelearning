...
namespace Samples.Dynamic.Trainers.Clustering
{
    public static class KMeansWithOptions
    {
        public static void Example()
        {
            ...
            var metrics = mlContext.Clustering.Evaluate(transformedTestData, "Label",
                "Score", "Features");
            ...
            Console.WriteLine(
                $"The first 3 coordinates of the first centroid are: ({string.Join(", ", centroids[0].GetValues().ToArray().Take(3))})");
            Console.WriteLine(
                $"The first 3 coordinates of the second centroid are: ({string.Join(", ", centroids[1].GetValues().ToArray().Take(3))})");
            ...
            int seed = 0)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                yield return new DataPoint
                {
                    ...
                    Features = Enumerable.Repeat(label, 50).Select(index =>
                            label == 0 ? randomFloat() + 0.1f : randomFloat() - 0.1f)
                        .ToArray()
                };
            }
        }
        ...
        {
            Console.WriteLine(
                $"Normalized Mutual Information: {metrics.NormalizedMutualInformation:F2}");
            Console.WriteLine($"Average Distance: {metrics.AverageDistance:F2}");
            Console.WriteLine(
                $"Davies Bouldin Index: {metrics.DaviesBouldinIndex:F2}");
        }
    }
}