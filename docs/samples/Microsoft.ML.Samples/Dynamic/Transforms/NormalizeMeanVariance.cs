...
namespace Samples.Dynamic
{
    public class NormalizeMeanVariance
    {
        public static void Example()
        {
            ...
            var normalize =
                mlContext.Transforms.NormalizeMeanVariance("Features", useCdf: true);
            ...
            var transformParams =
                normalizeTransform.GetNormalizerModelParameters(0) as
                    CdfNormalizerModelParameters<ImmutableArray<float>>;
            Console.WriteLine(
                $"The 1-index value in resulting array would be produce by:");
            Console.WriteLine(
                $" y = 0.5* (1 + ERF((x- {transformParams.Mean[1]}) / ({transformParams.StandardDeviation[1]} * sqrt(2)))");
            ...
            var noCdfParams =
                normalizeNoCdfTransform.GetNormalizerModelParameters(0) as
            ...
            Console.WriteLine(
                $"Values for slot 1 would be transfromed by applying y = (x - ({offset})) * {scale}");
            // Expected output:
