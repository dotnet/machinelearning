    ...
    class MapKeyToVector
    {
        ...
        public static void Example()
        {
            ...
            var pipeline = mlContext.Transforms.Conversion.MapKeyToVector("TimeframeVector", "Timeframe")
                           ...
                           .Append(mlContext.Transforms.Conversion.MapKeyToVector("PartsCount", "Parts", outputCountVector: true))
                           .Append(mlContext.Transforms.Conversion.MapKeyToVector("PartsNoCount", "Parts"));
            ...
            Console.WriteLine($"Timeframe  TimeframeVector    PartsCount  PartsNoCount");
            foreach (var featureRow in features)
                Console.WriteLine($"{featureRow.Timeframe}          {string.Join(',', featureRow.TimeframeVector.Select(x=>x))}  " +
                    $"{string.Join(',', featureRow.PartsCount.Select(x => x))}       {string.Join(',', featureRow.PartsNoCount.Select(x => x))}");
