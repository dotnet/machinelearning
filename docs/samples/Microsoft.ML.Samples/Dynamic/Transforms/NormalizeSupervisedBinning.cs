    ...
    public class NormalizeSupervisedBinning
    {
        public static void Example()
        {
            ...
            var samples = new List<DataPoint>()
            {
                ...
                new DataPoint()
                    {Features = new float[4] {4, -8, 1, 3}, Bin = "Bin3"},
                new DataPoint()
                    {Features = new float[4] {2, -5, -1, 4}, Bin = "Bin3"}
            };
            ...
            Console.WriteLine(
                $"y = (Index(x) / {transformParams.Density[0]}) - {(transformParams.Offset.Length == 0 ? 0 : transformParams.Offset[0])}");
            Console.WriteLine(
                "Where Index(x) is the index of the bin to which x belongs");
            Console.WriteLine(
                $"Bins upper borders are: {string.Join(" ", transformParams.UpperBounds[0])}");
            // Expected output:
            //  The 1-index value in resulting array would be produce by:
            //  y = (Index(x) / 2) - 0
            //  Where Index(x) is the index of the bin to which x belongs
            ...
            Console.WriteLine(
                $"The 1-index value in resulting array would be produce by:");
            Console.WriteLine(
                $" y = (Index(x) / {fixZeroParams.Density[1]}) - {(fixZeroParams.Offset.Length == 0 ? 0 : fixZeroParams.Offset[1])}");
            Console.WriteLine(
                "Where Index(x) is the index of the bin to which x belongs");
            Console.WriteLine(
                $"Bins upper borders are: {string.Join(" ", fixZeroParams.UpperBounds[1])}");
            // Expected output:
            //  The 1-index value in resulting array would be produce by:
            //  y = (Index(x) / 2) - 0.5
            //  Where Index(x) is the index of the bin to which x belongs
