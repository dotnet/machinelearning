    ...
    public static class ProduceWordBags
    {
        public static void Example()
        {
            ...
            var samples = new List<TextData>()
            {
                ...
                {
                    Text =
                        "ML.NET's ProduceWordBags API produces bag-of-word features from input text."
                },
                new TextData()
                {
                    Text =
                        "It does so by first tokenizing text/string into words/tokens then "
                },
                new TextData()
                    {Text = "computing n-grams and their neumeric values."},
                new TextData()
                {
                    Text =
                ...
                new TextData()
                {
                    Text = "the number of times n-gram occured in the data (Tf), or"
                },
                new TextData()
                {
                    Text =
                        "the inverse of the number of documents contain the n-gram (Idf),"
                },
                new TextData()
                    {Text = "or compute both and multipy together (Tf-Idf)."},
            };
            ...
            Console.WriteLine(
                $"Number of Features: {prediction.BagOfWordFeatures.Length}");
            ...
            var BagOfWordFeaturesColumn =
                transformedDataView.GetColumn<VBuffer<float>>(
                    transformedDataView.Schema["BagOfWordFeatures"]);
