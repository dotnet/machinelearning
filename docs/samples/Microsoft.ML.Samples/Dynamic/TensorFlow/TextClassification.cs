    ...
    public static class TextClassification
    {
        ...
        public static void Example()
        {
            ...
            var lookupMap = mlContext.Data.LoadFromTextFile(
                Path.Combine(modelLocation, "imdb_word_index.csv"),
                columns: new[]
                {
            ...
            var model = mlContext.Transforms.Text
                .TokenizeIntoWords("TokenizedWords", "Sentiment_Text")
                .Append(mlContext.Transforms.Conversion.MapValue(
                    "VariableLenghtFeatures", lookupMap,
            ...
            var engine = mlContext.Model
                .CreatePredictionEngine<IMDBSentiment, OutputScores>(model);

            // Predict with TensorFlow pipeline.
            var prediction = engine.Predict(data[0]);

            Console.WriteLine("Number of classes: {0}",
                prediction.Prediction.Length);
            Console.WriteLine("Is sentiment/review positive? {0}",
                prediction.Prediction[1] > 0.5 ? "Yes." : "No.");
            Console.WriteLine("Prediction Confidence: {0}",
                prediction.Prediction[1].ToString("0.00"));
        ...
        public class IMDBSentiment
        {
            ...
            /// Variable length vectors are produced by applying operations such as 'TokenizeWords'
            /// on strings
        ...
        public class IntermediateFeatures
        {
            ...
            [VectorType(MaxSentenceLenth)] public int[] Features { get; set; }
        }
        ...
        class OutputScores
        {
            [VectorType(2)] public float[] Prediction { get; set; }
        }
    }
}