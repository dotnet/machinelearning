...
namespace Samples.Dynamic
{
    public static class Cache
    {
        public static void Example()
        {
            ...
            Console.WriteLine(
                $"Lines={lines}, averageOfColumn0={columnAverage:0.00} and took {elapsedSeconds} seconds.");
            ...
            (lines, columnAverage, elapsedSeconds) =
                TimeToScanIDataView(mlContext, cachedData);
            Console.WriteLine(
                $"Lines={lines}, averageOfColumn0={columnAverage:0.00} and took {elapsedSeconds} seconds.");
            // Expected output (time is approximate):
            ...
            TimeToScanIDataView(MLContext mlContext, IDataView data)
        {
            ...
            var enumerable =
                mlContext.Data.CreateEnumerable<DatasetUtils.HousingRegression>(data,
                    reuseRowObject: true);
            ...
            foreach (var row in enumerable)
            {
                ...
                columnAverage += row.MedianHomeValue + row.CrimesPerCapita +
                                 row.PercentResidental + row.PercentNonRetail +
                                 row.CharlesRiver
                                 + row.NitricOxides + row.RoomsPerDwelling +
                                 row.PercentPre40s + row.EmploymentDistance
