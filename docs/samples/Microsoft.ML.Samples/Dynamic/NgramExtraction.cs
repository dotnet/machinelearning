...
namespace Samples.Dynamic
{
    public static partial class TransformSamples
    {
        public static void Example()
        {
            ...
            IEnumerable<Microsoft.ML.SamplesUtils.DatasetUtils.SampleSentimentData>
                data = Microsoft.ML.SamplesUtils.DatasetUtils.GetSentimentData();
            ...
            var charsPipeline =
                ml.Transforms.Text.TokenizeIntoCharactersAsKeys("Chars",
                    "SentimentText", useMarkerCharacters: false);
            var ngramOnePipeline =
                ml.Transforms.Text.ProduceNgrams("CharsUnigrams", "Chars",
                    ngramLength: 1);
            var ngramTwpPipeline =
                ml.Transforms.Text.ProduceNgrams("CharsTwograms", "Chars");
            ...
            var transformedData_onechars =
                oneCharsPipeline.Fit(trainData).Transform(trainData);
            var transformedData_twochars =
                twoCharsPipeline.Fit(trainData).Transform(trainData);

            // Small helper to print the text inside the columns, in the console. 
            Action<string, IEnumerable<VBuffer<float>>, VBuffer<ReadOnlyMemory<char>>
            ...
            var charsOneGramColumn =
                transformedData_onechars.GetColumn<VBuffer<float>>(
                    transformedData_onechars.Schema["CharsUnigrams"]);
            ...
            var charsTwoGramColumn =
                transformedData_twochars.GetColumn<VBuffer<float>>(
